...
function ENT:HitEntity(other, tr)

   local hitEnt = tr.Entity
   if not hitEnt:IsPlayer() and not ragmod:IsRagmodRagdoll(hitEnt) then return end
...
if SERVER then
  function ENT:Think()
     ...
     if tr.Hit and tr.HitNonWorld and IsValid(tr.Entity) then
        ...
        if other:IsPlayer() or ragmod:IsRagmodRagdoll(other) then
           self:HitEntity(other, tr)
        end
     end
